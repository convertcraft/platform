...
function flattenResponse(response) {
    ...
    if (nestedData !== null && typeof nestedData === 'object' && !Array.isArray(nestedData)) {
        Object.assign(result, nestedData);
    }
    return result;
}
function selectedFieldsResponse(data, selectedFields) {
    const picked = {};
...
function formatOutput(response, outputMode, selectedFields) {
    if (outputMode === 'raw') {
        return response;
    }
...
function getUserFacingError(error, itemIndex) {
    var _a;
    const fallback = {
        message: `The conversion request could not be completed [item ${itemIndex}]`,
        description: 'Check that the file URL is publicly accessible, that required parameters are set, and that the selected format is supported, then retry.',
    };
    if (!error || typeof error !== 'object') {
        return fallback;
    }
    const maybeError = error;
    const statusCode = Number((_a = maybeError.statusCode) !== null && _a !== void 0 ? _a : 0);
    ...
    if (statusCode === 400 && responseMessage.toLowerCase().includes('url')) {
        return {
            ...
            description: 'Check that the URL is publicly accessible and try again. If the file is behind authentication, use a pre-signed URL.',
        };
    }
    if (statusCode === 401 || statusCode === 403) {
        return {
            message: `The ConvertCraft credentials were rejected [item ${itemIndex}]`,
            description: 'Check the API Key and RapidAPI Host in your ConvertCraft API credentials, then run the node again.',
        };
    }
    if (statusCode === 404) {
        return {
...
class ConvertCraft {
    ...
    async execute() {
        ...
        const baseUrl = String((_a = credentials.baseUrl) !== null && _a !== void 0 ? _a : 'https://convertcraft-tools.p.rapidapi.com').replace(/\/+$/, '');
        for (let i = 0; i < items.length; i++) {
            try {
                const resource = this.getNodeParameter('resource', i);
                const operation = this.getNodeParameter('operation', i);
                const fileUrl = this.getNodeParameter('fileUrl', i);
                const outputMode = this.getNodeParameter('outputMode', i, 'simplified');
                const selectedFields = this.getNodeParameter('selectedFields', i, ['outputUrl']);
                const endpointPathByResource = {
                    pdf: {
                        convert: '/pdf-to-word',
                ...
                if (resource === 'pdf' && operation === 'compress') {
                    body.compressionLevel = this.getNodeParameter('compressionLevel', i, 'medium');
                }
                ...
                if (resource === 'image' && operation === 'resize') {
                    ...
                    const fit = this.getNodeParameter('fit', i, 'contain');
                    body.width = width;
                    body.height = height;
                    body.fit = fit;
                    body.w = width;
...
exports.ConvertCraft = ConvertCraft;
